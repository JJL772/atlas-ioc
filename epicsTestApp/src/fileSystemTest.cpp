 ...
 * ----------------------------------------------------------------------------
 * Description: File system data integrity checker thing
 * ----------------------------------------------------------------------------
...
static struct thread_param_s*
create_thread_param(struct test_s* test, size_t sz, const char* dir)
{
...
static void
fs_start_test(const iocshArgBuf* buf)
{
...
static void
fs_stop_tests(const iocshArgBuf* buf)
{
...
fsTestReport() 
{
  for (test_s* s = s_tests; s; s = s->next) {
    ...
    printf(
      "%s, %d iterations and %d failures, %.2f%% fail rate, write %.2f MiB/s, read %.2f MiB/s. (%s)\n",
      s->file,
      s->num,
      s->failures,
...
static void
fs_report_test(const iocshArgBuf* buf) {
  fsTestReport();
}
...
fs_test_thread(void* param)
{
  ...
  if (fd < 0) {
    printf("File %s failed to open after 10 retries, giving up!\n", p->filename);
    return;
  }
  ...
  if (!buf) {
    printf("Failed to malloc working buf, giving up!\n");
    return;
  }

  printf("Opened %s, starting test. requested size is %zu bytes\n", p->filename, p->fsz);
  ...
  while(p->test->run) {
    ...
    // Write out test data
    {
      ...
      if (toWrite > 0) {
        ++p->test->failures;
        EVERY_X(10, printf("%s write failed (%s): %s\n", get_time(tb, sizeof(tb)), p->filename, strerror(errno)));
        epicsThreadSleep(ERROR_SLEEP);
        continue;
      }
      ...
      // Finally, flush
      if (fsync(fd) > 0) {
        EVERY_X(10, printf("%s fsync failed (%s): %s\n", get_time(tb, sizeof(tb)), p->filename, strerror(errno)));
        ++p->test->failures;
        epicsThreadSleep(ERROR_SLEEP);
        continue;
      }
    ...
    // Read and validate data
    {
      ...
      while(toRead > 0) {
        ...
        if (ret < 0) {
          EVERY_X(10, printf("%s read failed (%s): %s\n", get_time(tb, sizeof(tb)), p->filename, strerror(errno)));
          ++p->test->failures;
          epicsThreadSleep(ERROR_SLEEP);
          break;
        }
        ...
        if (!memcheck_fast(buf, p->test->num, ret)) {
          EVERY_X(10, printf("%s data failed to validate %d expected, but got something else (%s)\n", get_time(tb,sizeof(tb)), p->test->num,
            p->filename));
          ++p->test->failures;
          epicsThreadSleep(ERROR_SLEEP);
        }
      ...
      if (p->test->num % 128 == 0) {
        printf(
          "%s: %d iterations, %d failures (%.2f%% fail rate), write %.2f MiB/s, read %.2f MiB/s\n", 
          p->test->file,
          p->test->num,
          p->test->failures,
  ...
  epicsGuard<epicsMutex> guard(p->test->mutex);
  printf("Test %s completed with %d iterations and %d failures, cleaning up...\n", p->filename, p->test->num, p->test->failures);
